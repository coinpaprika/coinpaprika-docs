...
### Global Data
- `Global.Get()` - Get global market overview

### Coins
- `Coins.List()` - List all coins
- `Coins.GetByID(coinID)` - Get coin by ID
- `Coins.GetTwitterTimelineByCoinID(coinID)` - Get coin's Twitter timeline
- `Coins.GetEventsByCoinID(coinID)` - Get coin events
- `Coins.GetMarketsByCoinID(coinID)` - Get coin markets
- `Coins.GetHistoricalOHLCVByCoinID(coinID, options)` - Get historical OHLCV data (Pro)

### Tickers
- `Tickers.List(options)` - Get all tickers
- `Tickers.GetByID(coinID, options)` - Get specific ticker
- `Tickers.GetHistoricalTickersByID(coinID, options)` - Get historical tickers (Pro)

### Exchanges
- `Exchanges.List(options)` - List all exchanges
- `Exchanges.GetByID(exchangeID, options)` - Get exchange by ID
- `Exchanges.GetMarketsByExchangeID(exchangeID, options)` - Get exchange markets

### People & Tags
- `People.GetByID(personID)` - Get person by ID
- `Tags.List(options)` - List tags
- `Tags.GetByID(tagID, options)` - Get tag by ID

### Search & Tools
- `Search.Search(options)` - Search coins, exchanges, people
- `PriceConverter.PriceConverter(options)` - Convert between currencies

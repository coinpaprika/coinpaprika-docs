...
- `getCoins()`: Lists all available coins.
- `getCoin(id)`: Retrieves details for a specific coin ID.
- `getCoinEvents(id)`: Gets events related to a coin.
- `getCoinExchanges(id)`: Gets exchanges where a coin is listed.
- `getCoinMarkets(id, quotes)`: Gets market data for a coin.
- `getCoinTweets(id)`: Gets the latest tweets for a coin.
- `getCoinOHLCV(id, quotes)`: Gets the latest OHLCV data.
- `getHistoricalCoinOHLCV(id, start, end, limit, quotes, interval)`: Gets historical OHLCV data.

### ExchangesService
Used to fetch data about exchanges.
- `getExchanges(quotes)`: Lists all exchanges.
- `getExchange(id, quotes)`: Retrieves details for a specific exchange.
- `getExchangeMarkets(id, quotes)`: Gets all markets for a specific exchange.

### FiatsService
Used to get fiat currency data.
- `getFiats()`: Lists all supported fiat currencies.

### GlobalStatsService
Used to get global market data.
- `getGlobalStats(quotes)`: Retrieves global cryptocurrency market statistics.

### NewsService
Used to fetch crypto news.
- `getNews(limit)`: Gets the latest news articles.

### PeopleService
Used to get information about people in the crypto space.
- `getPerson(id)`: Retrieves details for a specific person by ID.

### RankingsService
Used to get ranking data.
- `getRankings(name, quotes)`: Retrieves various rankings (e.g., top coin movers).

### SearchService
Used to perform searches.
- `search(query, categories, page, limit)`: Searches for coins, exchanges, people, and more.

### TagsService
Used to get information about tags.
- `getTags(sortBy)`: Lists all tags.
- `getTag(id)`: Retrieves details for a specific tag.

### TickersService
Used to fetch ticker price and market data.
- `getTicker(id, quotes)`: Retrieves the ticker for a specific coin ID.
- `getTickers(quotes, page)`: Lists tickers for all coins.

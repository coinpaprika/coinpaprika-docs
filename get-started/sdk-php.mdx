...
### `getTickerByCoinId(string $id): Ticker`
Get a specific ticker by coin ID (e.g., `'btc-bitcoin'`).

### `getHistoricalTickerByCoinId(string $id, string $start, string $interval): array`
Get historical ticker data for a coin. **(Pro API Key Required)**

### `getCoins(): array`
List all active coins.

### `getIcos(array $parameters = []): array`
Get a list of ICOs.

### `getOHLCV(string $id, string $interval): OHLCV`
Get the latest OHLCV data for a coin.

### `getTokenMeta(string $id): CoinMeta`
Get metadata for a specific token (e.g., links, whitepaper).

### `search(string $query, array $categories = null, int $limit = null): Search`
Search across coins, exchanges, ICOs, people, and tags.

### `setApiKey(string $apiKey)`
Set the API key after client initialization.

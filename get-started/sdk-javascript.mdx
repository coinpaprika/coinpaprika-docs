...
### `getCoins()`
List all coins available on Coinpaprika.

### `getCoin(coinId)`
Get a specific coin by its ID (e.g., `'btc-bitcoin'`).

### `getAllTickers(options)`
Get tickers for all coins or a specific coin.
- **`options`** `(Object)`:
  - **`coinId`** `(String)`: ID of a specific coin.
  - **`quotes`** `(Array<String>)`: An array of quote currencies to return (e.g., `['USD', 'BTC']`).
  - **`historical`** `(Object)`: Object for historical ticker data.
...
### `getTicker(options)`
DEPRECATED. Get information on all tickers or a specified ticker.
